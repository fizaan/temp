...
public class Register {
	...
	public static void main(String args[]) {
		...
		System.out.println(reg.addBytes(7, 3) + " ==> " +
			Bit.binStr(reg.getF(),true));
		
		System.out.println(reg.addBytes(7, 14) + " ==> " +
				Bit.binStr(reg.getF(),true));
		
		System.out.println(reg.addBytes(7, 60) + " ==> " +
				Bit.binStr(reg.getF(),true));
		
		System.out.println(reg.subBytes(20, 40) + " ==> " +
				Bit.binStr(reg.getF(),true));
		
		System.out.println(reg.subBytes(20, 20) + " ==> " +
				Bit.binStr(reg.getF(),true));
		
		System.out.println(reg.addBytes(20, -20) + " ==> " +
				Bit.binStr(reg.getF(),true));
		
		System.out.println(Integer.toBinaryString(reg.getF()));
		System.out.println(Bit.isSetInt(reg.getF(), Z+Bit.b25));
		Bit.setbits(Bit.b8);
		System.out.println(Bit.isSetInt(reg.getF(), Z+Bit.b1));
	}
	
	public String state() {
		/*StringBuilder sb = new StringBuilder();
		sb.append("PC[" + PC + "], ");
		sb.append("F[" + String.valueOf(flags) + "], ");
		sb.append("A[" + Bit.hexStr(A) + "], ");
		sb.append("B[" + Bit.hexStr(B) + "], ");
		sb.append("C[" + Bit.hexStr(C) + "], ");
		sb.append("D[" + Bit.hexStr(D) + "], ");
		sb.append("E[" + Bit.hexStr(E) + "]\n");
		sb.append("H[" + Bit.hexStr(H) + "], ");
		sb.append("L[" + Bit.hexStr(L) + "], ");
		sb.append("SP[" + Bit.hexStr(SP) + "], ");
		sb.append("HL[" + Bit.hexStr(getHL()) + "]\n");
		*/
		...
		//return String.format("Opcode=%04x, A=%04x, F=%04x, AF=%04x, BC=%04x, DE=%04x, HL=%04x, SP=%04x, PC=%04x, %s", opcode, A, F, getAF(), getBC(), getDE(), getHL(), getSP(), getPC(), flagsStr());
		//return sb.toString();
	}
