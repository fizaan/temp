...
import gameboyconcept.logo.GBNintendoLogo;
...
public class GBCpuBootRom implements CPUBusInterface{
		...
		while(true) {
			...
			if(reg.getPC() == 0x00c)
				...
				if(reg.getF()==0) {
					...
					Logit.info(this,"Displaying image...");
					
					/*
					 * choose 256 x 256 or 160 x 144
					 * Warning: 160 x 144 is not working
					 */
					GBNintendoLogo.init(bus, 256, 256);
				}
				else
	...
	public void H20() {
		...
		if(!(flags < 0 || ((flags & 0xFF) >> 7) == 1))
			...
			//Logit.wrCPUSTATE("Jump condition failed PC: " + reg.getPC());
		}
	}
	...
	*/
	...
	public void HC9() {
		...
		/*Logit.info(this,"Loops: " + Logit.loop);
